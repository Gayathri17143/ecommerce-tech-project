  ...
  let getUsers = async () => {
    try {
      const users = await axios.get(
        `https://63a9bccb7d7edb3ae616b639.mockapi.io/users?page=${currentPage}&limit=${usersPerPage}&search=${searchQuery}`
      );
  ...
  let handleDelete = async (id) => {
    try {
      ...
      if (confirmDelete) {
        ...
        getUsers();
        getTotalUsers(); // Update total users count after deletion
      }
    } catch (error) {
      console.log(error);
    }
    ...
    <>
      ...
      <div className="card shadow mb-4">
        ...
        <div className="card-body">
          {isLoading ? (
            <img src="https://media.giphy.com/media/ZO9b1ntYVJmjZlsWlm/giphy.gif" alt="" />
          ) : (
