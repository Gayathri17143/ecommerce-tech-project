    ...
    <React.Fragment>
      ...
      <section className="product-details">
        ...
        <div className="product-page-details">
          ...
          <p style={{color:"green",fontWeight:"700"}}>{product.availability}</p>
          ...
          <div className="cart-btns">
            <a href="#!" className="add-cart" onClick={() => addProductToCart(product)}>
              Add to Cart
            </a>
      ...
      <section className="product-all-info">
        <ul className="product-info-menu">
          {product.infos.map((info) => (
            <li key={info.title} onClick={() => setInfoTitle(info.title)} className={`p-info-list ${info.title === infoTitle ? 'active': ''}`}>
              {info.title}
            </li>
          ))}
        </ul>
