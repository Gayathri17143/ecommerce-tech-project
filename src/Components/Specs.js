...
const SpecsTable = () => {
    const specs = {
        general: [
            { label: "Network Type", value: "2G/3G/4G/5G/VoLTE supported" },
            { label: "SIM Type", value: "Dual SIM (nano SIM and eSIM)" },
        ],
        dimensions: [
            { label: "Width x Height x Depth", value: "76.7 x 159.9 x 8.25 mm" },
    ...
    const renderTable = (title, data) => (
        <div>
            <h2 style={{paddingLeft:"0px",fontSize:"20px",fontWeight:"600"}}>{title}</h2>
            <table>
                <thead>
                    <tr style={{border:"1px solid #ccc" ,opacity:"0.7"}}>
                        <th style={{padding:"5px",borderRight:"1px solid #ccc",opacity:"0.7"}}>Feature</th>
                        <th style={{padding:"5px",borderRight:"1px solid #ccc",opacity:"0.7"}}>Details</th>
                    </tr>
                </thead>
                <tbody>
                    {data.map((item, index) => (
                        <tr style={{border:"1px solid #ccc" ,opacity:"0.7"}} key={index}>
                            <td style={{padding:"5px",borderRight:"1px solid #ccc",opacity:"0.7"}}>{item.label}</td>
                            <td style={{padding:"5px",borderRight:"1px solid #ccc",opacity:"0.7"}}>{item.value}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
    ...
    return (
        <div>
            ...
            {renderTable("Dimensions", specs.dimensions)}
        </div>
    );
};
